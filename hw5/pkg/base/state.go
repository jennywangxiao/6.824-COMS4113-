...
func (s *State) HandleMessage(index int, deleteMessage bool) (result []*State) {
	//TODO: implement it
	states := make([]*State, 0)
	mess := s.Network[index]
	nodes := s.GetNode(mess.To()).MessageHandler(mess)
	for _,n := range nodes {
		state := s.Inherit(HandleEvent(mess))
		if deleteMessage == true {
			state.DeleteMessage(index)
		}
		state.UpdateNode(mess.To(),n)
...
func (s *State) NextStates() []*State {
	...
	for i := range s.Network {
		...
		// (use HandleMessage)
		if s.isDuplicate {
			states := s.HandleMessage(i, false)
			nextStates = append(nextStates, states...)
...
func (s *State) TriggerNodeTimer(address Address, node Node) []*State {
	...
	if IsNil(nodes) == false {
		for _,n := range nodes {
			state := s.Inherit(TriggerEvent(address, n.NextTimer()))
			state.UpdateNode(address, n)
			states = append(states, state)
		}
	}
	return states
...
func (s *State) RandomNextState() *State {
	...
	if roll < len(s.Network) {
		...
		if s.isLocalCall(roll) {
			return s.HandleMessage(roll, true)[0]
		} else if s.isDropOff {
			state := s.Inherit(DropOffEvent(s.Network[roll]))
			state.DeleteMessage(roll)
			return state
		} else if s.isDuplicate {
			return s.HandleMessage(roll, false)[0]
		} else {
			return s.HandleMessage(roll,true)[0]
		}
	}
