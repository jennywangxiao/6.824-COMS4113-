...
func Test1(t *testing.T) {
  ...
  // conclude primary is down even though it's pinging? No the primary changes
  ...
  
  {
    ...
    // expect p=3,b=1 - no ack, cannot change to a new view
  }
  fmt.Printf("  ... Passed\n")

  // if old servers die, check that a new (uninitialized) server
  // cannot take over. - meaning neither primary nor backup.
...

  {
    ...
    vz, _ := ck2.Get()
    // p won't be 2, due to no ack after it becomes backup
